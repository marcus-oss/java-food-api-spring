...
public class CadastroRestauranteIT {
	...
	private static final String VIOLACAO_DE_REGRA_DE_NEGOCIO_PROBLEM_TYPE = "Violacao de regra de negocio";
	...
	public void deveRetornarRespostaEStatusCorretos_QuandoConsultarRestauranteExistente() {
		given().pathParam("restauranteCodigo", burgerTopRestaurante.getCodigo()).accept(ContentType.JSON).when()
				.get("/{restauranteCodigo}").then().statusCode(HttpStatus.OK.value())
				.body("nome", equalTo(burgerTopRestaurante.getNome()));
	}

	@Test
	public void deveRetornarStatus404_QuandoConsultarRestauranteInexistente() {
		given().pathParam("restauranteCodigo", RESTAURANTE_ID_INEXISTENTE).accept(ContentType.JSON).when()
				.get("/{restauranteCodigo}").then().statusCode(HttpStatus.NOT_FOUND.value());
	}
	...
	private void prepararDados() {
		...
		burgerTopRestaurante.setTaxaFrete(new BigDecimal(8));
		...
		comidaMineiraRestaurante.setTaxaFrete(new BigDecimal(8));
